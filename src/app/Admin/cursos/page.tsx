...
import {getImages_talleresAdmin, uploadImageCursos,} from "@/services/repoImage";
...
//para subir imagenes:
import { handleUploadCursoImage, handleDeleteCursoImage} from "@/helpers/repoImagesCurso";
...
const Cursos: React.FC = () => {
  ...
  const handleUploadAndFetchImages = async (selectedFile: File | null) => {
    const result = await handleUploadCursoImage(selectedFile, cursoDetails.imagen || "");
    if (result.error) {
  ...
  const fetchImages = async () => {
    ...
    if (result.error) {
      ...
      const updatedCursos = cursos.map((curso) => {
        // Asegurarse de que curso.imagen tenga un valor predeterminado
        const cursoImagen = curso.imagen || "";
        const imageIndex = result.images.findIndex(
          (image: string) => image === cursoImagen
        );
        if (imageIndex !== -1) {
          return { ...curso, imageUrl: result.downloadurls[imageIndex] };
        }
        return curso;
      });
  ...
  async function handleEliminarCurso(id: number) {
    try {
      //Ventana de confirmación para eliminar el curso
       console.log("CURSO DETAILS EN ELIM: ", cursoDetails);
  ...
  return (
    <main className="relative min-h-screen w-screen">
      ...
      />
      <div
        className="fixed justify-between w-full p-4"
        style={{ background: "#1CABEB" }}
      >
        <Navigate />
      </div>
      ...
      <div
        className="fixed bottom-0 mt-20 bg-white w-full"
        style={{ opacity: 0.66 }}
      >
        <But_aside />
      </div>
